...
var LivingRoom = (function ($, window, undefined) {
    ...
    function implementFlexSlider4Sketc() {
        var count, sketcsEl, imgElm, statusBox, statusBoxCtrl;
        count = 0;
        sketcsEl = $('#sketcs-slide');
        statusBoxCtrl = sketcsEl.find('[data-status-box-control]');
        imgElm = sketcsEl.find('img');
        statusBox = sketcsEl.find('[data-status-box]');
        statusBoxGroup =  sketcsEl.find('[data-status-box],[data-status-box-backdrop]');
        // statusBoxBackdrop = sketcsEl.find('');
        ...
        imgElm.each(function () {
            ...
            img.onload = function () {
                ...
                if (lengthImg === count) {
                    ...
                    statusBoxGroup.removeClass('hidden');
        ...
        statusBoxGroup.on('click.hide', function() {
            statusBoxGroup.addClass('hidden');
            statusBox.addClass('fixed-top');
            statusBoxCtrl.removeClass('hidden');
        });
        statusBoxCtrl.on('click.showStatus', function() {
            var statusShowTimeOut = null,
            el = $(this);
            el.hide();
            statusBox.fadeIn();
