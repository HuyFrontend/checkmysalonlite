...
//             label = me.tagName === 'LABEL' ? me : parent.tagName === 'LABEL' ? parent : null; // the .btn label
...
//       // fix self.indicator has no method addEventListener on IE8
//       // self.indicator && self.indicator.addEventListener( "click", self.indicatorHandler, false);
//       self.indicator && self.indicator.addEventListenerOrAttachEvent( self.indicatorHandler, "click", false);
...
(function(){
  //MODAL DEFINITION
    var Modal = function(element, options) { // element is the trigger button / options.target is the modal
      ...
      this.timer = 0;
      // add new prototype changeContentModal
      this.options.newContent = options.newContent;
      this.options.replaceContainer = options.replaceContainer;
      this.options.replaceHref = options.replaceHref;
      this.init();
    }
    ...
    Modal.prototype.init = function() {
      ...
      // condition for change content of modal
      if ( this.options.newContent && this.options.newContent !== undefined && this.options.replaceContainer && this.options.replaceContainer !== undefined) {
        this.changeContentModal( this.options.newContent, this.options.replaceContainer );
      }
      this.dismiss();
      this.keydown();
      this.trigger();
    ...
    Modal.prototype._open = function() {
      ...
      if(this.options.replaceHref) {
        this.modal.querySelector('[data-href]').setAttribute('data-href',this.options.replaceHref);
      }
      this.opened = true;
    }
    ...
    Modal.prototype._close = function() {
      ...
      //add condition
      if(this.options.replaceHref) {
        this.modal.querySelector('[data-href]').setAttribute('data-href','');
      }
    ...
    // add new protorype
    Modal.prototype.changeContentModal = function( newContent, replaceContainer ) {
      return this.modal.querySelector('[' + replaceContainer + ']').innerHTML = newContent;
    }
    ...
    Modal.prototype.keydown = function() {
      ...
      // document.addEventListenerOrAttachEvent(function(e) {
      //   if (self.options.keyboard && e.which == 27) {
      //     self.close();
      //   }
      // }, 'keydown' );
    ...
    Modal.prototype.trigger = function() {
      ...
      for ( i;i<tgl;i++ ) {
        ...
        // triggers[i].addEventListenerOrAttachEvent(function(e) {
        //   var b = e.target || e.srcElement,
        //   s = b.getAttribute('data-target') && b.getAttribute('data-target').replace('#','')
        //    || b.getAttribute('href') && b.getAttribute('href').replace('#','');
        //   if ( document.getElementById( s ) === self.modal ) {
    ...
    Modal.prototype.resize = function() {
      ...
      // window.addEventListenerOrAttachEvent( function() {
      //   setTimeout(function() {
      //     self._resize()
      //   }, 100)
      // }, 'resize', false);
    ...
    Modal.prototype.dismiss = function() {
      ...
      this.modal.addEventListener('click', function(e){
       var me = e.target || e.srcElement;
        if ( me.parentNode.getAttribute('data-dismiss') === 'modal' || me.getAttribute('data-dismiss') === 'modal' || me === self.modal ) {
          self.close();
        }
      });
      // this.modal.addEventListenerOrAttachEvent( function (e){
      //   var me = e.target || e.srcElement;
      //   if ( me.parentNode.getAttribute('data-dismiss') === 'modal' || me.getAttribute('data-dismiss') === 'modal' || me === self.modal ) {
      //     self.close();
      //   }
  ...
  for (var i = 0, mdl = Modals.length; i < mdl; i++ ) {
    ...
    options.replaceHref = modal.getAttribute('data-href');
...
//         var me = e.target || e.srcElement;
//         if (self.popover && me === self.popover.querySelector('.close')) {
//           self.close();
//         }
//       },
//       this.close = function(e) {
...
//         var next = e.target || e.srcElement; //the tab we clicked is now the next tab
