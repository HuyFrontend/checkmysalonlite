  ...
  var validateFn = {
    ...
    required: function(el){
      var rule = getRuleOfEl.call(this, el),
          val = $.trim(el.val()),
          holderValue = el.attr('placeholder');
      if(!val || val == rule.init || val == holderValue){
        return false;
      }
    },
    checked: function(el){
      ...
      formVL
        ...
        .on({
          'submit.validate': function(evt){
            ...
            // formVL.find('.' + errorInputClass).first().focus(); // Virtual keyboard bug
            if(allIsValid){
              return options.onSubmit.call(that);
            }
            else{
              return options.onFailed.call(that);
            }
          },
